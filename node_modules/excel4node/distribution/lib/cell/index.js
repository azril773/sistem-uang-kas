...
function stringSetter(val) {
  ...
  if (typeof val !== 'string') {
    logger.warn('Value sent to String function of cells %s was not a string, it has type of %s', JSON.stringify(this.excelRefs), _typeof(val));
    val = '';
  }
...
function numberSetter(val) {
  if (val === undefined || parseFloat(val) !== val) {
    throw new TypeError(util.format('Value sent to Number function of cells %s was not a number, it has type of %s and value of %s', JSON.stringify(this.excelRefs), _typeof(val), val));
  }
...
function booleanSetter(val) {
  if (val === undefined || typeof (val.toString().toLowerCase() === 'true' || (val.toString().toLowerCase() === 'false' ? false : val)) !== 'boolean') {
    throw new TypeError(util.format('Value sent to Bool function of cells %s was not a bool, it has type of %s and value of %s', JSON.stringify(this.excelRefs), _typeof(val), val));
  }
...
function formulaSetter(val) {
  if (typeof val !== 'string') {
    throw new TypeError(util.format('Value sent to Formula function of cells %s was not a string, it has type of %s', JSON.stringify(this.excelRefs), _typeof(val)));
  }
...
function dateSetter(val) {
  ...
  var dtStyle = new Style(this.ws.wb, {
...
function styleSetter(val) {
  ...
  this.cells.forEach(function (c) {
    ...
    if (c.s === 0) {
      ...
      var newStyleOpts = deepmerge(curStyle.toObject(), thisStyle);
...
var cellBlock = /*#__PURE__*/function () {
  function cellBlock() {
