...
var _addSheetPr = function _addSheetPr(promiseObj) {
  ...
  return new Promise(function (resolve, reject) {
    var o = promiseObj.ws.opts; // Check if any option that would require the sheetPr element to be added exists

    if (o.pageSetup.fitToHeight !== null || o.pageSetup.fitToWidth !== null || o.outline.summaryBelow !== null || o.outline.summaryRight !== null || o.autoFilter.ref !== null) {
...
var _addDimension = function _addDimension(promiseObj) {
  ...
  return new Promise(function (resolve, reject) {
    var firstCell = 'A1';
    var lastCell = "".concat(utils.getExcelAlpha(promiseObj.ws.lastUsedCol)).concat(promiseObj.ws.lastUsedRow);
    var ele = promiseObj.xml.ele('dimension');
    ele.att('ref', "".concat(firstCell, ":").concat(lastCell));
    ele.up();
    resolve(promiseObj);
  });
};
...
var _addSheetViews = function _addSheetViews(promiseObj) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var sv = ele.ele('sheetView').att('showGridLines', o.showGridLines).att('workbookViewId', o.workbookViewId).att('rightToLeft', o.rightToLeft).att('zoomScale', o.zoomScale).att('zoomScaleNormal', o.zoomScaleNormal).att('zoomScalePageLayoutView', o.zoomScalePageLayoutView);
...
var _addAutoFilter = function _addAutoFilter(promiseObj) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (typeof o.startRow === 'number') {
      ...
      ele.att('ref', "".concat(startCell, ":").concat(endCell));
...
var _addLegacyDrawing = function _addLegacyDrawing(promiseObj) {
  return new Promise(function (resolve, reject) {
    var rId = promiseObj.ws.relationships.indexOf('commentsVml') + 1;

    if (rId === 0) {
      resolve(promiseObj);
    } else {
      promiseObj.xml.ele('legacyDrawing').att('r:id', 'rId' + rId).up();
      resolve(promiseObj);
    }
  });
};
...
var _addPageBreaks = function _addPageBreaks(promiseObj) {
  // colBreaks (§18.3.1.14); rowBreaks (§18.3.1.74)
  var rowBreaks = promiseObj.ws.pageBreaks.row;

  if (rowBreaks.length > 0) {
    var rbEle = promiseObj.xml.ele('rowBreaks');
    rbEle.att('count', rowBreaks.length);
    rbEle.att('manualBreakCount', rowBreaks.length);
    rowBreaks.forEach(function (pos) {
      var bEle = rbEle.ele('brk');
      bEle.att('id', pos);
      bEle.att('man', 1);
      bEle.up();
    });
  ...
  var colBreaks = promiseObj.ws.pageBreaks.column;

  if (colBreaks.length > 0) {
    var cbEle = promiseObj.xml.ele('colBreaks');
    cbEle.att('count', colBreaks.length);
    cbEle.att('manualBreakCount', colBreaks.length);
    colBreaks.forEach(function (pos) {
      var bEle = cbEle.ele('brk');
      bEle.att('id', pos);
      bEle.att('man', 1);
      bEle.up();
    });
...
var sheetXML = function sheetXML(ws) {
  return new Promise(function (resolve, reject) {
    ...
    var wsXML = xml.begin({
      ...
      xmlString += chunk;
    }).ele('worksheet').att('mc:Ignorable', 'x14ac').att('xmlns', 'http://schemas.openxmlformats.org/spreadsheetml/2006/main').att('xmlns:mc', 'http://schemas.openxmlformats.org/markup-compatibility/2006').att('xmlns:r', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships').att('xmlns:x14ac', 'http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac'); // Excel complains if specific elements on not in the correct order in the XML doc as defined in §M.2.2
    ...
    _addSheetPr(promiseObj).then(_addDimension).then(_addSheetViews).then(_addSheetFormatPr).then(_addCols).then(_addSheetData).then(_addSheetProtection).then(_addAutoFilter).then(_addMergeCells).then(_addConditionalFormatting).then(_addDataValidations).then(_addHyperlinks).then(_addPrintOptions).then(_addPageMargins).then(_addLegacyDrawing).then(_addPageSetup).then(_addPageBreaks).then(_addHeaderFooter).then(_addDrawing).then(function (promiseObj) {
      return new Promise(function (resolve, reject) {
        wsXML.end();
        resolve(xmlString);
      });
  ...
  return new Promise(function (resolve, reject) {
    ...
    ws.relationships.forEach(function (r, i) {
      ...
      if (r instanceof hyperlinks.Hyperlink) {
        ...
        relXML.ele('Relationship').att('Id', rId).att('Target', '../comments' + ws.sheetId + '.xml').att('Type', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/comments');
      } else if (r === 'commentsVml') {
        relXML.ele('Relationship').att('Id', rId).att('Target', '../drawings/commentsVml' + ws.sheetId + '.vml').att('Type', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/vmlDrawing');
      }
    });
...
var commentsXML = function commentsXML(ws) {
  return new Promise(function (resolve, reject) {
    var commentsXml = xml.create('comments', {
      'version': '1.0',
      'encoding': 'UTF-8',
      'standalone': true,
      'allowSurrogateChars': true
    ...
    var commentList = commentsXml.ele('commentList');
    Object.keys(ws.comments).forEach(function (ref) {
      commentList.ele('comment').att('ref', ref).att('authorId', '0').att('guid', ws.comments[ref].uuid).ele('text').ele('t').text(ws.comments[ref].comment);
    });
    var xmlString = commentsXml.doc().end();
    resolve(xmlString);
  });
};

var commentsVmlXML = function commentsVmlXML(ws) {
  return new Promise(function (resolve, reject) {
    // do not add XML prolog to document
    var vmlXml = xml.begin().ele('xml');
    ...
    var sl = vmlXml.ele('o:shapelayout').att('v:ext', 'edit');
    sl.ele('o:idmap').att('v:ext', 'edit').att('data', ws.sheetId);
    var st = vmlXml.ele('v:shapetype').att('id', '_x0000_t202').att('coordsize', '21600,21600').att('o:spt', '202').att('path', 'm,l,21600r21600,l21600,xe');
    st.ele('v:stroke').att('joinstyle', 'miter');
    st.ele('v:path').att('gradientshapeok', 't').att('o:connecttype', 'rect');
    Object.keys(ws.comments).forEach(function (ref) {
      var _ws$comments$ref = ws.comments[ref],
          row = _ws$comments$ref.row,
          col = _ws$comments$ref.col,
          position = _ws$comments$ref.position,
          marginLeft = _ws$comments$ref.marginLeft,
          marginTop = _ws$comments$ref.marginTop,
          width = _ws$comments$ref.width,
          height = _ws$comments$ref.height,
          zIndex = _ws$comments$ref.zIndex,
          visibility = _ws$comments$ref.visibility,
          fillColor = _ws$comments$ref.fillColor;
      var shape = vmlXml.ele('v:shape');
      shape.att('id', "_".concat(ws.sheetId, "_").concat(row, "_").concat(col));
      shape.att('type', "#_x0000_t202");
      shape.att('style', "position:".concat(position, ";margin-left:").concat(marginLeft, ";margin-top:").concat(marginTop, ";width:").concat(width, ";height:").concat(height, ";z-index:").concat(zIndex, ";visibility:").concat(visibility));
      shape.att('fillcolor', fillColor);
      shape.att('o:insetmode', 'auto');
      shape.ele('v:path').att('o:connecttype', 'none');
      var tb = shape.ele('v:textbox').att('style', 'mso-direction-alt:auto');
      tb.ele('div').att('style', 'text-align:left');
      var cd = shape.ele('x:ClientData').att('ObjectType', 'Note');
