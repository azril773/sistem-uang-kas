...
var addRootContentTypesXML = function addRootContentTypesXML(promiseObj) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    promiseObj.wb.sheets.forEach(function (s, i) {
      xml.ele('Override').att('ContentType', 'application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml').att('PartName', "/xl/worksheets/sheet".concat(i + 1, ".xml"));
...
var addWorkbookXML = function addWorkbookXML(promiseObj) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var workbookViewEle = booksViewEle.ele('workbookView'); // bookViews (§18.2.1)
    ...
    promiseObj.wb.sheets.forEach(function (s, i) {
      var sheet = sheetsEle.ele('sheet').att('name', s.name).att('sheetId', i + 1).att('r:id', "rId".concat(i + 1));
      ...
      if (s.printArea) {
        var name = s.name;
        var startCellRef = "$".concat(utils.getExcelAlpha(s.printArea.startCol), "$").concat(s.printArea.startRow);
        var endCellRef = "$".concat(utils.getExcelAlpha(s.printArea.endCol), "$").concat(s.printArea.endRow);
        s.wb.definedNameCollection.addDefinedName({
          name: '_xlnm.Print_Area',
          localSheetId: s.localSheetId,
          refFormula: "'".concat(name, "'!").concat(startCellRef, ":").concat(endCellRef)
        });
      }
    });
...
var addWorkbookRelsXML = function addWorkbookRelsXML(promiseObj) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    xml.ele('Relationship').att('Id', "rId".concat(promiseObj.wb.sheets.length + 1)).att('Target', 'sharedStrings.xml').att('Type', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/sharedStrings');
    xml.ele('Relationship').att('Id', "rId".concat(promiseObj.wb.sheets.length + 2)).att('Target', 'styles.xml').att('Type', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles');
    promiseObj.wb.sheets.forEach(function (s, i) {
      xml.ele('Relationship').att('Id', "rId".concat(i + 1)).att('Target', "worksheets/sheet".concat(i + 1, ".xml")).att('Type', 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet');
    });
    var xmlString = xml.doc().end(promiseObj.xmlOutVars);
      ...
      if (thisSheet) {
        ...
        thisSheet.generateXML().then(function (xml) {
          return new Promise(function (resolve) {
            ...
            promiseObj.xlsx.folder('xl').folder('worksheets').file("sheet".concat(curSheet, ".xml"), xml);
            resolve();
          });
        }).then(function () {
          ...
          return new Promise(function (resolve) {
            if (xml) {
              promiseObj.xlsx.folder('xl').folder('worksheets').folder('_rels').file("sheet".concat(curSheet, ".xml.rels"), xml);
            }
          ...
          return new Promise(function (resolve) {
            if (xml) {
              promiseObj.xlsx.folder('xl').file("comments".concat(curSheet, ".xml"), xml);
            }
          ...
          return new Promise(function (resolve) {
            if (xml) {
              promiseObj.xlsx.folder('xl').folder('drawings').file("commentsVml".concat(curSheet, ".vml"), xml);
            }
...
var addStylesXML = function addStylesXML(promiseObj) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    var cellStyleXfsXML = xml.ele('cellStyleXfs').att('count', 1);
    cellStyleXfsXML.ele('xf').att('numFmtId', '0').att('fontId', '0').att('fillId', '0').att('borderId', '0').att('applyFont', 'true').att('applyBorder', 'false').att('applyAlignment', 'false').att('applyProtection', 'false');
    ...
    var cellStylesXML = xml.ele('cellStyles').att('count', 1);
    cellStylesXML.ele('cellStyle').att('name', 'Normal').att('xfId', '0').att('builtinId', '0');
